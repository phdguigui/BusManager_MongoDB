    ...
    public class LineView
    {
        ...
        private static void ShowAllLines()
        {
            ...
            if (lineList is not null && lineList?.Count > 0)
            {
                ...
                {
                    Console.WriteLine($"{line.Id.ToString().PadRight(26)}" +
                        $"{line.Name.PadRight(30)}" +
                        $"{line.Code.PadRight(10)}" +
                        $"{line.Origin.PadRight(20)}" +
                        $"{line.Destiny.PadRight(25)}" +
                        $"{(line.Active ? "Sim" : "Não")}");
                }
                ...
                else
                {
                    ...
                    for (int i = 0; i < stopCount; i++)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (tempId is null || _stationService.GetStationById(ObjectId.Parse(tempId)) is null)
                            {
                                Console.Clear();
                        ...
                        Stop stop = new Stop()
                        {
                            ...
                            StationId = ObjectId.Parse(tempId),
                            Hour = horaChegada
                        };
        ...
        private static void ShowUpdateLine()
        {
            ...
            Console.Clear();
            Console.WriteLine("Carregando...");
            var line = _service.GetLineById(id is null ? ObjectId.Parse("0") : ObjectId.Parse(id));
        ...
        private static void ShowStopsByLine ()
        {
            ...
            Console.Clear();
            Console.WriteLine("Carregando...");
            var line = _service.GetLineById(ObjectId.Parse(lineId));
            ...
            if (line is not null)
            {
                ...
                var rota = _stopService.GetRouteLine(ObjectId.Parse(lineId));
        ...
        private static void ShowDeleteStop()
        {
            ...
            if (line is not null)
            {
                ...
                Console.WriteLine($"Digite o ID da estação");
                var tempId = Console.ReadLine();
                var station = _stationService.GetStationById(ObjectId.Parse(tempId));
                if (station is null)
                {
                    Console.Clear();
                    Console.WriteLine($"Estação com id {tempId} não encontrada");
